...
def detect_and_move_duplicates(
    ...
    """
    Convenience function to detect and optionally move duplicate files.

    Args:
        prefix: S3 prefix to search for duplicates
        bucket: S3 bucket name
        auto_move: If True, automatically move duplicates to destination folder
        destination_folder: Target folder for duplicates (default: DuplicateCheck)
        exclude_entities: List of entity names to exclude from duplicate check (e.g., ['RHUM'])
                         These entities need to process older files first and then run again.

    Returns:
        Dict with duplicate detection and move results
    """
    s3_client = boto3.client('s3')

    # Find duplicates
    result = find_exact_duplicates_s3(
        bucket_name=bucket,
        prefix=prefix,
        s3_client=s3_client,
    ...
    response = {
        ...
        'files_kept': [],
        'move_errors': [],
        'excluded_entities': exclude_entities or [],
        'excluded_files_count': 0
    }

    if auto_move and (result.total_duplicates > 0 or result.total_superseded > 0):
        # Filter out excluded entities from move operation
        exclude_entities_upper = [e.upper() for e in (exclude_entities or [])]

        if exclude_entities_upper:
            # Filter superseded groups to exclude specified entities
            filtered_superseded = []
            excluded_files = []

            if result.superseded_groups:
                for group in result.superseded_groups:
                    # Check if entity matches any excluded entity
                    entity = group.entity.upper() if group.entity else ''
                    # Also check the file type for entity patterns like RHUM75, RHUM
                    file_type_upper = group.file_type.upper() if group.file_type else ''

                    is_excluded = False
                    for excl in exclude_entities_upper:
                        if excl in entity or excl in file_type_upper:
                            is_excluded = True
                            # Track excluded files
                            excluded_files.extend(group.superseded_files)
                            break

                    if not is_excluded:
                        filtered_superseded.append(group)

            response['excluded_files_count'] = len(excluded_files)
            response['excluded_files'] = excluded_files[:20]  # Just first 20 for reference

            # Create a modified result with filtered superseded groups
            from dataclasses import replace
            filtered_result = DuplicateCheckResult(
                total_files=result.total_files,
                unique_files=result.unique_files,
                duplicate_groups=result.duplicate_groups,
                total_duplicates=result.total_duplicates,
                storage_waste_bytes=result.storage_waste_bytes,
                groups=result.groups,  # Keep exact duplicates as-is
                superseded_groups=filtered_superseded,
                total_superseded=sum(len(g.superseded_files) for g in filtered_superseded)
            )
        ...
        response['files_moved'] = move_result.get('files_moved', [])
        response['files_kept'] = move_result.get('files_kept', [])
        response['move_errors'] = move_result.get('errors', [])
